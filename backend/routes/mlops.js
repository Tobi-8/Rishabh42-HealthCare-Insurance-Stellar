...
router.post('/pipelines',
  ...
  async (req, res, next) => {
    try {
      res.status(201).json(await mlops.createPipeline({ ...req.body, created_by: req.user?.id }));
    } catch (err) { next(err); }
  }
);

router.get('/pipelines', async (req, res, next) => {
  try { res.json(await mlops.listPipelines()); } catch (err) { next(err); }
});

router.post('/pipelines/:pipelineId/train',
  param('pipelineId').isUUID(), validate,
  async (req, res, next) => {
    try {
      res.status(202).json(await mlops.triggerTraining(req.params.pipelineId, req.user?.id));
    } catch (err) {
...
router.get('/pipelines/:pipelineId/runs',
  ...
  validate,
  async (req, res, next) => {
    try { res.json(await mlops.listRuns(req.params.pipelineId, req.query.limit)); } catch (err) { next(err); }
  }
);

router.get('/runs/:runId',
  param('runId').isUUID(), validate,
  async (req, res, next) => {
    try {
...
router.post('/deployments',
  ...
  async (req, res, next) => {
    try {
      res.status(201).json(await mlops.deployModel(req.body.model_id, req.body.environment, req.user?.id));
    } catch (err) {
      if (err.message.includes('not found')) return res.status(404).json({ error: err.message });
      next(err);
    }
...
router.get('/deployments',
  ...
  validate,
  async (req, res, next) => {
    try { res.json(await mlops.listDeployments(req.query.environment)); } catch (err) { next(err); }
  }
);

router.post('/deployments/:deploymentId/rollback',
  param('deploymentId').isUUID(), validate,
  async (req, res, next) => {
    try {
...
router.get('/models/:modelId/health',
  ...
  validate,
  async (req, res, next) => {
    try { res.json(await mlops.getModelHealth(req.params.modelId, req.query.hours || 24)); } catch (err) { next(err); }
  }
);
...
router.post('/pipelines/:pipelineId/retrain',
  ...
  async (req, res, next) => {
    try {
      res.status(202).json(await mlops.scheduleRetraining(
        req.params.pipelineId, req.body.trigger_type, req.user?.id, req.body.reason || null
      ));
    } catch (err) {
      if (err.message.includes('not found')) return res.status(404).json({ error: err.message });
      next(err);
...
router.get('/pipelines/:pipelineId/retraining-history',
  param('pipelineId').isUUID(), validate,
  async (req, res, next) => {
    try { res.json(await mlops.listRetrainingHistory(req.params.pipelineId)); } catch (err) { next(err); }
  }
);
...
router.post('/experiments',
  ...
  validate,
  async (req, res, next) => {
    try { res.status(201).json(await mlops.createExperiment(req.body)); } catch (err) { next(err); }
  }
);

router.get('/experiments/:experimentId/results',
  param('experimentId').isUUID(), validate,
  async (req, res, next) => {
    try {
...
router.patch('/experiments/:experimentId/status',
  ...
  validate,
  async (req, res, next) => {
    try { res.json(await mlops.updateExperimentStatus(req.params.experimentId, req.body.status)); } catch (err) { next(err); }
  }
);
...
router.get('/models/:modelName/versions',
  param('modelName').isString().notEmpty(), validate,
  async (req, res, next) => {
    try { res.json(await mlops.getModelVersions(req.params.modelName)); } catch (err) { next(err); }
  }
);
