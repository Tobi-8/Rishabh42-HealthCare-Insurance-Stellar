...
class MLOpsService {
  getDb() { return new sqlite3.Database(DB_PATH); }
  ...
  async createPipeline(data) {
    ...
    try {
      await this.run(db,
        `INSERT INTO mlops_pipelines (pipeline_id, name, model_type, config, status, created_by, created_at)
  ...
  async _runTraining(runId, pipelineId, pipeline) {
    ...
    try {
      ...
      await this.run(db,
        `INSERT INTO ml_models (model_id, name, version, model_type, description, status, created_at)
         VALUES (?, ?, ?, ?, ?, 'staging', CURRENT_TIMESTAMP)`,
        [modelId, pipeline.name, `v${Date.now()}`, pipeline.model_type, `Trained by pipeline ${pipelineId}`]
      );
      await this.run(db,
        `INSERT INTO mlops_explainability (explain_id, model_id, run_id, method, feature_importance, created_at)
      ...
      await this.run(db,
        `UPDATE mlops_runs SET status = 'completed', model_id = ?, metrics = ?, finished_at = CURRENT_TIMESTAMP WHERE run_id = ?`,
  ...
  async _autoDeploy(db, modelId, pipelineId, runId, modelName) {
    ...
    await this.run(db,
      `INSERT INTO mlops_deployments (deployment_id, model_id, pipeline_id, run_id, environment, status, deployed_at)
  ...
  async deployModel(modelId, environment, deployedBy) {
    ...
    try {
      ...
      await this.run(db,
        `INSERT INTO mlops_deployments (deployment_id, model_id, pipeline_id, run_id, environment, status, deployed_by, deployed_at)
  ...
  async recordFeedback(modelId, predictionId, actual, predicted, latencyMs) {
    ...
    try {
      await this.run(db,
        `INSERT INTO mlops_feedback (feedback_id, model_id, prediction_id, actual_value, predicted_value, latency_ms, recorded_at)
      ...
      if (count && count.cnt % 100 === 0) setImmediate(() => this._checkDrift(modelId));
    } finally { db.close(); }
  }
  ...
  async _checkDrift(modelId) {
    ...
    try {
      const recent = await this.all(db,
        `SELECT actual_value, predicted_value FROM mlops_feedback WHERE model_id = ? ORDER BY recorded_at DESC LIMIT 100`,
        [modelId]
      );
      const window200 = await this.all(db,
        `SELECT actual_value, predicted_value FROM mlops_feedback WHERE model_id = ? ORDER BY recorded_at DESC LIMIT 200`,
        [modelId]
      );
      const baseline = window200.slice(100);
      const accuracy = rows => rows.length ? rows.filter(r => r.actual_value === r.predicted_value).length / rows.length : null;
      ...
      if (recentAcc !== null && baselineAcc !== null && baselineAcc - recentAcc > 0.05) {
        ...
        await this.run(db,
          `INSERT INTO mlops_drift_alerts (alert_id, model_id, drift_score, severity, baseline_accuracy, recent_accuracy, detected_at)
           VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`,
          [uuidv4(), modelId, parseFloat(drift.toFixed(4)), drift > 0.15 ? 'high' : 'medium',
  ...
  async scheduleRetraining(pipelineId, triggerType, triggeredBy, reason) {
    ...
    try {
      await this.run(db,
        `INSERT INTO mlops_retraining_schedule (schedule_id, pipeline_id, trigger_type, triggered_by, reason, status, scheduled_at)
  ...
  async createExperiment(data) {
    ...
    try {
      ...
      await this.run(db,
        `INSERT INTO mlops_experiments (experiment_id, name, description, variants, status, start_date, end_date, created_at)
         VALUES (?, ?, ?, ?, 'active', ?, ?, CURRENT_TIMESTAMP)`,
        [experimentId, data.name, data.description || null, JSON.stringify(data.variants),
